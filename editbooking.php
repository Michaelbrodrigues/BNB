...
//prepare a query and send it to the server ## this is the querry for the room dropdown
...
//save the room data if the error flag is still clear and room id is > 0
    if ($error == 0 and $id > 0) {
        $query = "UPDATE booking SET roomID=?,Checkin=?,Checkout=?,ContactNumber=?,BookingExtras=? WHERE roomID=?";
        $stmt = mysqli_prepare($DBC,$query); //prepare the query
        mysqli_stmt_bind_param($stmt,'issssi', $roomID, $Checkin, $Checkout, $ContactNumber,$BookingExtras,$id); 
